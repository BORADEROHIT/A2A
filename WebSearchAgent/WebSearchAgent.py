...
class WebsearchAgent:
    ...
    async def stream(self, query, context_id)-> AsyncIterable[dict[str, Any]]:
        ...
        for item in self.graph.stream(inputs,config=config,stream_mode='values'):
            ...
            event=item['messages'][-1]
            pprint(event)
            if isinstance(event, AIMessage):
                if hasattr(event,'tool_calls') and event.tool_calls:
                    yield{
                        'is_task_complete':False,
                        'content':"Looking for information in the open internet..."
                    }
                else:
                    yield{
                        'is_task_complete':True,
                ...
                yield{
                    'is_task_complete':False,
                    'content':"Working on tool response..."
                }
...
def main():
    ...
    response = agent.invoke("what is current rate of doller in indian rupees (INR)",
                            uuid.uuid4())
    pprint(response)
